...
class Simulation:
    def __init__(self, measured_states=[0, 1], tlr=1.2, tlf=0.8, num_ref=3):
        self._measured_states = measured_states
        self._tlr = tlr
        self._tlf = tlf
        self._num_ref = num_ref

    def run(self, lr=1.2, lf=0.8) -> float:
        sorted_tracks = np.argsort(reference[:,1])
        train_tracks = reference[sorted_tracks[:self._num_ref],0].astype(int) #[58798, 18806, 90297]
        MPC_HORIZON = 10
        plt.ion()
        plt.show()
